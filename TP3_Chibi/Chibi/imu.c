...
void imu_compute_units(void) {
	...
	for (uint8_t axis = 0; axis < NB_AXIS; ++axis) {
		imu_values.acceleration[axis] = (float) (imu_values.acc_raw[axis]
				- imu_values.acc_offset[axis]) * ACC_RAW2G * STANDARD_GRAVITY;
		imu_values.gyro_rate[axis] = (float) (imu_values.gyro_raw[axis]
				- imu_values.gyro_offset[axis]) * DEG2RAD(GYRO_RAW2DPS);
	}
}
...
static THD_FUNCTION(imu_reader_thd, arg) {
	...
	while (chThdShouldTerminateX() == false) {
		...
		if (imu_configured == true) {
			...
			mpu9250_read(imu_values.gyro_raw, imu_values.acc_raw,
					&imu_values.status);
...
void imu_start(void) {
	...
	imuThd = chThdCreateStatic(imu_reader_thd_wa, sizeof(imu_reader_thd_wa),
	NORMALPRIO, imu_reader_thd, NULL);
}
...
void imu_compute_offset(messagebus_topic_t *imu_topic, uint16_t nb_samples) {
	 ...
	 *   TASK 9 : TO COMPLETE
	 */
	int16_t acc_sum[NB_AXIS];
	int16_t gyro_sum[NB_AXIS];

	for (int16_t i = 0; i < nb_samples; ++i) {
		messagebus_topic_wait(imu_topic, &imu_values, sizeof(imu_values));

		for (uint8_t axis = 0; axis < NB_AXIS; ++axis) {
			acc_sum[axis] += imu_values.acc_raw[axis];
			gyro_sum[axis] += imu_values.gyro_raw[axis];
		}
	}

	imu_values.acc_offset[X_AXIS] = acc_sum[X_AXIS] / nb_samples;
	imu_values.acc_offset[Y_AXIS] = acc_sum[Y_AXIS] / nb_samples;
	imu_values.acc_offset[Z_AXIS] = acc_sum[Z_AXIS] / nb_samples;
	imu_values.gyro_offset[X_AXIS] = gyro_sum[X_AXIS] / nb_samples;
	imu_values.gyro_offset[Y_AXIS] = gyro_sum[Y_AXIS] / nb_samples;
	imu_values.gyro_offset[Z_AXIS] = gyro_sum[Z_AXIS] / nb_samples;

	// If we want to compensate gravity
