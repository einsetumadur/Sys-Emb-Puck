...
void gpio_config_output_af_pushpull(GPIO_TypeDef *port, unsigned int pin, unsigned int alternate_function)
{
	// Output type pushpull : OTy = 0
	port->OTYPER &= ~(1 << pin);
	...
	if (pin < 8) // Set alternate_function bits in AFRL
	{
		port->AFR[0] = (GPIOD->AFR[0] & ~(0b1111 << (pin * 4))) | (alternate_function << (pin * 4));
	}
	else if (pin < 16) // Set alternate_function bits in AFRH
	{
		port->AFR[1] = (GPIOD->AFR[1] & ~(0b1111 << ((pin - 8) * 4))) | (alternate_function << ((pin - 8) * 4));
	}
}
